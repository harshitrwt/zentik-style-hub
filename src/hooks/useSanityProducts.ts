...
export interface SanityProduct {
  _id: string;
  name: string;
  slug: string; // must match frontend /products/:slug
  price: number;
  originalPrice?: number;
  discount?: number;
  images: { url: string; alt?: string }[];
...
export const useProducts = () =>
  useQuery<SanityProduct[]>({
    queryKey: ['products'],
    queryFn: async () => {
      try {
        const sanityProducts: SanityProduct[] = await sanityClient.fetch(productsQuery);
        if (sanityProducts?.length) return sanityProducts;
        return fallbackProducts.map(transformFallbackProduct);
      } catch (err) {
...
export const useBestSellers = () =>
  useQuery<SanityProduct[]>({
    ...
    queryFn: async () => {
      try {
        const sanityProducts: SanityProduct[] = await sanityClient.fetch(bestSellersQuery);
        if (sanityProducts?.length) return sanityProducts;
        return fallbackProducts.filter(p => p.isBestSeller).map(transformFallbackProduct);
      } catch (err) {
...
export const useNewArrivals = () =>
  useQuery<SanityProduct[]>({
    ...
    queryFn: async () => {
      try {
        const sanityProducts: SanityProduct[] = await sanityClient.fetch(newArrivalsQuery);
        if (sanityProducts?.length) return sanityProducts;
        return fallbackProducts.filter(p => p.isNew).map(transformFallbackProduct);
      } catch (err) {
...
// Fetch product by slug (key for /products/:slug page)
export const useProductBySlug = (slug: string) =>
  useQuery<SanityProduct | null>({
    queryKey: ['product', slug],
    queryFn: async () => {
      if (!slug) return null;
      try {
        const product: SanityProduct = await sanityClient.fetch(productBySlugQuery, { slug });
...
export const useProductsBySubcategory = (subcategory: string) =>
  useQuery<SanityProduct[]>({
    ...
    queryFn: async () => {
      ...
      try {
        const sanityProducts: SanityProduct[] = await sanityClient.fetch(productsBySubcategoryQuery, { subcategory });
        if (sanityProducts?.length) return sanityProducts;
        return fallbackProducts.filter(p => p.subcategory === subcategory).map(transformFallbackProduct);
      } catch (err) {
...
// --- HELPERS ---
export const getProductImageUrl = (product: SanityProduct, index = 0): string =>
  product.images?.[index]?.url || product.images?.[0]?.url || '/placeholder.svg';

export const isSizeInStock = (product: SanityProduct, sizeLabel: string): boolean =>
  product.sizes?.find(s => s.label === sizeLabel)?.inStock ?? false;
